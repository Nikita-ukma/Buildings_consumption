...
MAE: 522.73 MW
R²: 0.8705
Winter RMSE: 795.20 MW
Spring RMSE: 542.75 MW
Summer RMSE: 949.38 MW
Fall RMSE: 680.58 MW
'''
...
# 3. Split data into training (before 2022) and test (2022) sets
def split_data(df, test_start='2022-01-01', test_end='2022-12-31'):
    train = df.loc[:test_start].iloc[:-1]  # up to the start of 2022, not including the test day itself
    test = df.loc[test_start:test_end]     # all of 2022
    return train, test
...
def train_model(X_train, y_train, X_val=None, y_val=None):
    params = {
        ...
        'metric': 'rmse',
        'num_leaves': 63,  # increased for better modeling of complex seasonality
    ...
    if X_val is not None and y_val is not None:
        ...
        model = lgb.train(
            params,
            train_data,
            num_boost_round=2000,  # increased for better convergence
            valid_sets=[train_data, val_data],
            callbacks=[lgb.early_stopping(stopping_rounds=50)]
        )
    else:
        # If no validation set, use the entire training set
        model = lgb.train(
            params,
            train_data,
...
if __name__ == "__main__":
    try:
        ...
        # Split into training (before 2022) and test (2022) sets
