...
# Переконуємося, що дата-час у правильному форматі
df['Datetime'] = pd.to_datetime(df['Datetime'])

# Перевірка дублікатів у даті
...
# Перевірка наявності пропущених значень і їх обробка
...
# Сортування індексу для гарантії хронологічного порядку
df = df.sort_index()
...
# 2. Перевірка регулярності даних та забезпечення повного часового ряду
# Визначення найчастішого інтервалу між вимірюваннями
time_diffs = df.index.to_series().diff().value_counts()
print("Найчастіші інтервали між вимірюваннями:")
print(time_diffs.head())

# Використаємо інтервал "1 година" для ресемплінгу
# Замість reindex використаємо asfreq, який створить регулярний часовий ряд
try:
    ...
    # Використовуємо ресемпльований датафрейм далі
    df = df_resampled
...
# Тижнева сезонність (168 годин = 7 днів * 24 години)
try:
...
# Річна сезонність - перевіряємо чи достатньо даних
if len(df) >= 8760:  # Приблизно рік годинних даних
    try:
...
# Функція для візуалізації компонентів
...
# 4. Аналіз циклічності (довгострокові коливання)
# Спробуємо визначити циклічність за допомогою згладжування
if has_daily:
    def extract_cyclical_component(data, window=30*24):  # вікно в годинах
        """Виділення циклічної компоненти за допомогою згладжування"""
        # Згладжуємо для отримання тренду
        trend = data.rolling(window=window, center=True).mean()
        # Видаляємо тренд
        detrended = data - trend
        # Видаляємо сезонну компоненту
        seasonal = daily_decomposition.seasonal
        # Циклічна компонента = детрендований ряд - сезонність
        # Приведемо індекси до спільного формату
    ...
    # Застосовуємо з різними вікнами згладжування
    window_sizes = [24*7, 24*30, 24*90]  # тиждень, місяць, квартал (в годинах)
...
# 5. Додатковий аналіз: розподіл значень і автокореляція
plt.figure(figsize=(15, 6))

# Гістограма розподілу енергоспоживання
plt.subplot(1, 2, 1)
...
# Збереження компонентів у CSV для подальшого аналізу
...
# Додатково: зберегти дані для звіту
